...
export class UserRepositoryMock implements UserRepository {
	...
	async update(user: UserEntity, request: UpdateUserRequest): Promise<UserEntity> {
	...
	async updatePassword(user: UserEntity, request: UpdateUserPasswordRequest): Promise<UserEntity> {
		const mergedUser = { ...user, ...request } as UserEntity;
	...
	mockDeleteThrowUserWithIdNotExistsException(id: string): void {
		this.mockDelete.mockImplementation(() => {
			throw new UserWithIdNotExistsException(id);
		});
	}
}
