...
export class TypeOrmEnvironmentArranger {
	...
	static async cleanDatabase(): Promise<void> {
		const teardownTypeOrmConfig: TypeOrmModuleOptions = {
			database: process.env.SKELETON_API_postgres_database_name,
			host: process.env.SKELETON_API_postgres_database_host,
			port: Number(process.env.SKELETON_API_postgres_database_port),
			username: process.env.SKELETON_API_postgres_database_username,
			password: process.env.SKELETON_API_postgres_database_password,
