...
import { userAudienceGuardLogs } from '../config/logs/user-audience-guard.logs';
...
const { className } = userAudienceGuardConstants;
const { requestLog } = userAudienceGuardLogs;
...
export class UserAudienceGuard implements CanActivate {
	...
	private checkUserAudiences(
		...
		if (!this.areValidAudiences(userAudiences, validAudiences)) {
			throw new InvalidUserAudienceException();
		}
	}
