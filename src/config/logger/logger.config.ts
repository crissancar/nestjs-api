...
export const loggerConfig: Params = {
	pinoHttp: {
		...
		customSuccessObject: (req: IncomingMessage, res: ServerResponse) => {
			...
			return { method, status: { code: statusCode, message: statusMessage } };
		},
		...
		customErrorObject: (req: IncomingMessage, res: ServerResponse) => {
			const { statusCode, statusMessage } = res;
			const { method } = req;
